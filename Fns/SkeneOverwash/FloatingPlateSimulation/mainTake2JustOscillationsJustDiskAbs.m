...
refincwaveN=@(x,t)I*real( (exp(k*(x))*exp(k*L/2) + abs(RN)*exp(-k*(x))*exp(-k*L/2))*exp(-1i*omega*t));
refincwave=@(x,t)I*real(exp(k*(x + L/2)) + RN*exp(-k*(x+L/2))*exp(-1i*omega*t));

dt = 0.01;
xwhole = linspace(-2*pi/(2*kr),2*pi/(2*kr),300);
time = 0;
...
hold off
plot(xwhole, refincwave(xwhole,time),'-k','LineWidth',2)
hold on;
plot(xwhole, refincwaveN(xwhole,time),'--r','LineWidth',2)
