...
for waveperiod=0.95%[0.8,0.9,1.0]
%select incident wave steepness
for amp=0:0.005:0.04%[0.04,0.06,0.08,0.1,0.12,0.14,0.15]

    clearvars -except amp waveperiod
...
% %new
% leftwaveN=@(x,t) I*real( (exp(k*(x+L/2)) + R*exp(-k*(x+L/2)))*exp(-1i*omega*t));
% rightwaveN=@(x,t)I*real(T*exp(k*(x-L/2))*exp(-1i*omega*t));
% incidentwaveN = @(x,t) I*real( (exp(k*(x+L/2)))*exp(-1i*omega*t));
% 
% beamheightN=@(j,t) I*real(1i/sqrt(alpha)*EtaOut(j)*exp(-1i*omega*t))+plateabove;

%function handle for the velocity of the water at the left and right of the
%plate.
leftvelocityN=@(x,t) I*real(g*k/(1i*omega)*( (exp(k*(x+L/2)) + R*exp(-k*(x+L/2)))*exp(-1i*omega*t) ));
rightvelocityN=@(x,t) I*real(g*k/(1i*omega)*(T*exp(k*(x-L/2))*exp(-1i*omega*t)));
...
%         subplot(2,1,2)
%         hold off
%         plot(x_left,leftwaveN(x_left,time))
%         hold on
%         plot(x_right,rightwaveN(x_right,time))
%         plot(xbeam,beamheightN(1:length(displacement),time))
...
%save result
save(strcat('test_',num2str(amp),'_',num2str(waveperiod),'_nohits_fixedmotion.mat'))
