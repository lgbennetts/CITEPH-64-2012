...
if ~exist('TestName','var');     TestName= '1' ;end;  %'14' , '6', '16'
...
% DispWindow = disp(and(tm > t0, tm < t1));
% DispWindowfilt = highpass(DispWindow,Tp/2.0,SamplingRate);
% [Pxx,f,Pxxc] = pwelch(DispWindowfilt,T_window,T_window/2, [],SamplingRate,'ConfidenceLevel',0.9);
...
% hold on;
% % plot(f*Tp,Pxx,'.-r','LineWidth',2, 'DisplayName', 'Welch', 'MarkerSize',12)
% % plot(f*Tp,Pxxc(:,1),'.-r','LineWidth',2, 'DisplayName', 'Welch', 'MarkerSize',12)
% % plot(f*Tp,Pxxc(:,2),'.-r','LineWidth',2, 'DisplayName', 'Welch', 'MarkerSize',12)
xlim([0,3]);
...
figure();
set(gca,'FontSize',18) 
plot(ffp,sqrt(2*MeanSpectra),'.-b','LineWidth',2, 'DisplayName', 'Mean Spectra', 'MarkerSize',12)
xlim([0,3]);
title(['Average Energy Amplitde Spectra between t = ',num2str(t0jj0),' [s] ',num2str(t1jj1),' [s] (',num2str(jj1 - jj0),' Time Windows) ', Description]);
xlabel('Frequency / Fp')
ylabel('A(f)')
legend();
...
hold on;
% p1a = plot(tm_vec,sqrt(2.*Sn_mat(jjT,:)),'-k', 'DisplayName', 'Peak Height Around','LineWidth',2);
