...
if ~exist('TestName','var'); TestName= {{'16','16'},{'15','15'},{'14','14'},{'17','17'}}; end ;  % 9, 14 %14 %2
...
%     hold off;
    ...
    figure();
    set(gca,'FontSize',18) 
    p1 = plot(ffpA, ATWR./m0R, '-r', 'DisplayName',  ['Conc ', num2str(conc{1}) ,' | Probes : ( ', num2str(probes{1}(1)),' , ', num2str(probes{1}(end)) ' ) | Hs ', num2str(Hs),' [m]'] , 'LineWidth', 2);   
    hold on;
    ATWRmaxplot = ATWRmax./m0R;
    ATWRminplot = ATWRmin./m0R;
    ShadeX = [ffpA; flipud(ffpA)];
    inBetween = [ATWRmaxplot; flipud(ATWRminplot)];
    patch(ShadeX, inBetween, 'r', 'LineStyle','none')
    alpha(0.3) 
    p1a = plot(ffpA, ATWRmaxplot, ':r', 'DisplayName', 'Maximum and Minimum' , 'LineWidth', 2);
    p1b = plot(ffpA, ATWRminplot, ':r' , 'LineWidth', 2);
    p2 = plot(ffpA, ATWT./m0T, '-b', 'DisplayName',  ['Conc ', num2str(conc{2}) ,' | Probes : ( ', num2str(probes{2}(1)),' , ', num2str(probes{2}(end)) ' ) | Hs ', num2str(Hs),' [m]'] , 'LineWidth', 2);
    ATWTmaxplot = ATWTmax./m0T;
    ATWTminplot = ATWTmin./m0T;
    ShadeX = [ffpA; flipud(ffpA)];
    inBetween = [ATWTmaxplot; flipud(ATWTminplot)];
    patch(ShadeX, inBetween, 'b', 'LineStyle','none')
    alpha(0.3) 
    p2a = plot(ffpA, ATWTmaxplot, ':b', 'DisplayName', 'Maximum and Minimum' , 'LineWidth', 2);
    p2b = plot(ffpA, ATWTminplot, ':b' , 'LineWidth', 2);
    p3 = plot(ffJSSmooth, JSSmooth / (Hs/4)^2, '-k', 'DisplayName',  ['JonSwap'] , 'LineWidth', 3);
    
    xlabel('Frequency / Fp');
    ylabel('S(f) / m0 ');
    xlim([0,3])
    title(['Spectra Comparison of Probes', Description]);
    legend([p1 p1a p1b p2 p2a p2b p3]);
    hold off; 
    ...
    [~,TpMSI] = max(ATWT);
    TpMST = 1.0/ffA(TpMSI);
    TperrorR = abs(TpMSR - Tp)/ Tp
    TperrorT = abs(TpMST - Tp)/ Tp
end
