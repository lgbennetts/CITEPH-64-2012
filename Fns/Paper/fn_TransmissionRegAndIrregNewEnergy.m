...
%Load wikiwaves
WikiWavesPRSATransmission = load('./Data/Gen/WikiWavesPRSATransmission');
...
%WL wikiwaves
for i = 1: length(WikiWavesPRSATransmission.Tps)
    ki = dispersion_free_surface((2*pi/WikiWavesPRSATransmission.Tps(i))^2/9.81,0,3.1);
    WLWW(i) = 2*pi/imag(ki);
end
...
EffectiveDiskNumber = c*5/0.99;
plot(WLWW, abs(WikiWavesPRSATransmission.T_GF).^(2*EffectiveDiskNumber), '--b' , 'LineWidth', 3);
PlotNames{end +1} = 'WikiWaves Greens Function';

plot(WLWW,abs(WikiWavesPRSATransmission.T_EM).^(2*EffectiveDiskNumber), '--k' , 'LineWidth', 3);
PlotNames{end +1} = 'WikiWaves Eigenmode Match';

plot(WLWW, abs(WikiWavesPRSATransmission.T_LR).^(2*EffectiveDiskNumber), '-r' , 'LineWidth', 3);
PlotNames{end +1} = 'Lucas Full';

plot(WLWW, abs(WikiWavesPRSATransmission.T_LRRem).^(2*EffectiveDiskNumber), '-b' , 'LineWidth', 3);
PlotNames{end +1} = 'Lucas Full No Draft or Surge';
