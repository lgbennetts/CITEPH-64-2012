...
% if ~exist('TestNames','var') TestNames ={'5'}; end 
...
%     Success
    ...
    ExpRef(j) = sqrt(2*outRef(IndexSatisfy).value);  
    ...
    ExpTra(j) = sqrt(2*outTrans(IndexSatisfy).value);  

    %Calibration
    outRefC = fn_SpectraAverageTgT(Tp,0,CalibTestName,WaveType,proberef,OutputStr);
    IndexSatisfy =  find(ismember({outTrans.name},'SpecFilt')) ;
    CalRef(j) = CalFac*sqrt(2*outRefC(IndexSatisfy).value); 
    ...
    CalTra(j) = CalFac*sqrt(2*outTransC(IndexSatisfy).value); 
