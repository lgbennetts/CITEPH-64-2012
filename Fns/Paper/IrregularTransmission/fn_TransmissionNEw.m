...
function fn_TransmissionNew(conc,TestName,probes)
...
if ~exist('OutputStr','var');  v='Tpers=fn_Tpers(Tp,WaveType);'; end
...
for j = 1: length(TestName)
    ...
    TpS = linspace(Tp/2,2*Tp,10);
    PerWholeCalib = [];
    SpectraWholeCalib = [];
    PerWholeConc = [];
    SpectraWholeConc = [];
    for ji = 2: length(TpS)
        TpC = (TpS(ji-1) + TpS(ji))/2;
    ...
    %Figure
%     figure();
%     plot(PerWholeCalib,SpectraWholeCalib,'-b');
%     hold on;
%     plot(PerWholeConc,SpectraWholeConc,'-r');
%     plot(1./ffJS,JSspec{j},'--k' );
