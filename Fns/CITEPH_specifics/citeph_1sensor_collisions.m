function [an_mat,Sn_mat,t_int] = citeph_1sensor_collisions(time,displ,T_target,outloc,inloc)
...
if nargin==0
   ...
   fname = [fdir,'23071525.a13/accelerometers/houle_reg_060.dat']
   A     = load(fname);{A}
   %%
   scale_fac   = 100;
...
if exist('outloc')
   ...
   outfile     = [outdir,'/collisions_',sensor_name,'.mat'];
...
for m=m0:Nwindows
   ...
   Tm02_vec(m) = xtra{3};
   %%
   As       = Hs_vec(m);%%significant acceleration amplitude;
   thresh   = min(19,1.5*As);%%acceleration threshold for collisions is relative to As;
   %{an_mat,an0}
   %{Sn_mat,S0}
   an_mat(:,m) = an0;
   Sn_mat(:,m) = S0;
