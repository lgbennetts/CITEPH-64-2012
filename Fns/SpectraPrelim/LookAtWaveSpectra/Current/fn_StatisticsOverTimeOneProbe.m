...
if ~exist('TestName','var');     TestName= '16' ;end;  %'14' , '6', '16'
...
h1=subplot(2,1,1);
surf(h1,tm_vec(:),ff,log10(sqrt(2*Sn_mat)))
shading interp
view(h1,[0,90])
...
h2=subplot(2,1,2);
plot(h2,ff,sqrt(2*MeanSpectra),'-b','LineWidth',2, 'DisplayName', 'Data')
hold on;
wJS = 2*pi/Tp;
hmJS =  Hm;
ffJS = 0:0.01:5;
...
plot(h2,ffJS,JSspec,'--k','LineWidth',2, 'DisplayName', ['JonSwap wp: ', num2str(wJS), '[Hz]' , ' Hs ($4\sqrt{m_0}$) ', num2str(round(hmJS,4)), '[m]' ])
...
h1=subplot(2,1,1);
surf(h1,tm_vec(:),ff,log10(Sn_mat))
shading interp
view(h1,[0,90])
xlim(h1,[tm_vec(1), tm_vec(end)])
ylim(h1,[0 1.0/ (Tp/4)]);
...
h2=subplot(2,1,2);
plot(h2,ff,MeanSpectra,'-b','LineWidth',2, 'DisplayName', 'Data')
hold on;
hmJS =  Hm;
ffJS = 0:0.01:5;
JSspec  = jonswap(2*pi.*ffJS,'wp',wJS,'Hs',hmJS);
