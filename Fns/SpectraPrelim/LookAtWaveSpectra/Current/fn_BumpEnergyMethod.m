...
function fn_BumpEnergyMethod(conc, probes,TestName)
...
if ~exist('Tpers','var');  Tpers=100; end
...
for TestName = TestNames
    ...
    if (test_Success == 1 && calib_Success ==1 )
        ...
        TargetRegion=[0.75*Tp,1.25*Tp];

        TWindow = Tpers*Tp;
        [Tps,TpsRegions] = fn_GetPeaksIncoming(calibconc ,calib_Name,1:10,Tp,TargetRegion,t0description,t1description, TWindow);

        for i = 1:length(Tps)
            TpI = Tps(i);
            TpsRegionI = TpsRegions(i,:);

            AvgEnergyInc = fn_ExtractEnergy(calibconc,calib_Name,probes,TpI,TpsRegionI,conc,t0description,t1description, TWindow);
            AvgEnergyOut = fn_ExtractEnergy(conc,TestName,probes,TpI,TpsRegionI,conc,t0description,t1description, TWindow);
