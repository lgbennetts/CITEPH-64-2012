...
function [Energy,Tps] = fn_ExtractAllPeaks(tm,disp,Tp,conc,PX,dt)

    %first one
    RemoveRegion = [100,2];
    RemovedRegions = [RemoveRegion];
...
if ~isempty(RemovedRegions)
    for j = 1:size(RemovedRegions,1)
        ...
        Sn(and(periods > RemovedRegions(j,2) ,periods < RemovedRegions(j,1) )) = 0;
    end
