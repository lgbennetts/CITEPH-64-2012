...
ProbeEnergy = zeros(5,10);
ProbePeaks = zeros(5,10);
...
%     end
    ...
    [Energy_Baseline,Peaks_Baseline,Regions_Baseline,TgTps_Baseline] = fn_ExtractAllPeaksProbe1(tm,disp,Tp,conc,ProbeLocXY(1),dt);
    clear ProbeLocXY tm disp c_pram WaveType;
    
    ProbeEnergy(1:length(Energy_Baseline),probei) = Energy_Baseline.';
    ProbePeaks(1:length(Peaks_Baseline),probei) = Peaks_Baseline.';
%     ProbePeaks(probei) = Peaks_Baseline;
end
