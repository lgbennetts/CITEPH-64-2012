...
if ~exist('WaveType','var');     WaveType= 'All';end %'Irregular %'Regular'
...
%   col_model=' ''b-.'' ';
  col_reg_peak=' ''b.'' , ''markersize'' , 12';
  col_reg_offpeak=' ''bx'' , ''markersize'' , 12';
  col_irreg_peak = ' ''r.'' , ''markersize'' , 12';
  col_irreg_offpeak = ' ''rx'' , ''markersize'' , 12';
% %   col_nl=' ''bo'' , ''markersize'' , 12';
% %   col_ii=' ''bx'' , ''markersize'' , 12';
% %   col_nl_ii=' ''bo'' , ''markersize'' , 12';
% %   coll=' ''b.'' , ''markersize'' , 12';
% %   collnl=' ''bo'' , ''markersize'' , 12';
  col_model=' ''b--'' '; %k:
%   col=' ''r.'' , ''markersize'' , 12';
...
%what_mod = 'Boltzmann steady';%'2d EMM';%'Boltzmann steady'
what_mod = 'Boltzmann steady';%'2d EMM';%'Boltzmann steady'
RemScat=0;
...
%     end 
    ...
    Types_vec = dum_dat.Types(dum_trans < 1);
    Index_vec = dum_dat.Index(dum_trans < 1);
 ...
 if DO_DATA
  ...
  for lp=1:length(hd)
   ...
   if strcmp(PLOT_TYP,'trans_coeff')
    ...
    %plot
    if ~isempty(xx(and(Index_vec == 1, strcmp(Types_vec,"Regular") )))  
      eval(['p(1,lp)=plot(hd(lp),xx(and(Index_vec == 1, strcmp(Types_vec,"Regular") )),trans_coeff(and(Index_vec == 1, strcmp(Types_vec,"Regular") )),' col_reg_peak ');']);  
    end
    
    if ~isempty(xx(and(Index_vec ~= 1, strcmp(Types_vec,"Regular") )))
        eval(['p(1,lp)=plot(hd(lp),xx(and(Index_vec ~= 1, strcmp(Types_vec,"Regular") )),trans_coeff(and(Index_vec ~= 1, strcmp(Types_vec,"Regular") )),' col_reg_offpeak ');']);
    end
    
    if ~isempty(xx(and(Index_vec == 1, strcmp(Types_vec,"Irregular") )))
       eval(['p(1,lp)=plot(hd(lp),xx(and(Index_vec == 1, strcmp(Types_vec,"Irregular") )),trans_coeff(and(Index_vec == 1, strcmp(Types_vec,"Irregular") )),' col_irreg_peak ');']);
    end
    
    if ~isempty(xx(and(Index_vec ~= 1, strcmp(Types_vec,"Irregular") )))
       eval(['p(1,lp)=plot(hd(lp),xx(and(Index_vec ~= 1, strcmp(Types_vec,"Irregular") )),trans_coeff(and(Index_vec ~= 1, strcmp(Types_vec,"Irregular") )),' col_irreg_offpeak ');']);
    end
